...
export async function seedMarketingPlan() {
  ...
  if (cnt >= 36) { console.log('⏭ marketing_plans: v2 уже есть'); return; }
  ...
  const tasks: [string, string, number][] = [
    ...
    // ГРАФИК
    ['schedule', 'День 1 (13 апр): фичи в прилу + таблица блогеров (Instagram #якутск #якутия + LabelUp + tgstat) + 5 демо + написать ВСЕМ', 1],
    ['schedule', 'День 2 (14 апр): утро VIRALMAXING 20-30 мин -> 2-3 идеи -> 1-3 Reels + согласовать рекламы + запустить в тот же день + вечером 15-25 новых блогеров', 2],
    ['schedule', 'День 3 (15 апр): 1-3 Reels + обработка ответов (кредиты, промокоды, сценарий) + комментарии неответившим + первые блогеры создают контент + 15-25 новых', 3],
    ['schedule', 'День 4 (16 апр): 1-3 Reels + первые выкладки блогеров выходят + проверить промокоды (активации, покупки) + 15-25 новых', 4],
    ['schedule', 'День 5 (17 апр): 1-3 Reels + 1 с ч/б фото (прогрев 9 мая) + анализ промокодов + вторая волна блогеров (фокус на работающие ниши) + 15-25 новых', 5],
    ['schedule', 'День 6 (18 апр): 1-3 Reels + полный анализ: DAU, покупки, выручка, ROI по промокодам + что работает -> удвоить + 15-25 новых', 6],
    ['schedule', 'День 7 (19 апр): ИТОГИ НЕДЕЛИ — выручка vs цель, ROI по каждому промокоду, какие блогеры/ниши сработали, стратегия на след. неделю', 7],
    ['schedule', 'КАЖДЫЙ ДЕНЬ: утро VIRALMAXING -> 1-3 Reels -> блогеры (бартер / бартер+оплата) -> вечер 15-25 новых. Если работает — продолжаем', 8],
